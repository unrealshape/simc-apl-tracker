...
| 7 | `conflagrate` | if=action.conflagrate.charges>=2 |
| 8 | `chaos_bolt` | if=talent.diabolic_ritual&(demonic_art\|(variable.ritual_length<action.chaos_bolt.execute_time))&target.health.pct>20 |
| 9 | `conflagrate` | if=soul_shard<=4.2&buff.backdraft.stack<1 |
| 10 | `summon_infernal` | — |
| 11 | `malevolence` | — |
...
| 15 | `conflagrate` | if=soul_shard<=4.4&buff.backdraft.stack<1 |
| 16 | `ruination` | — |
| 17 | `cataclysm` | if=talent.lake_of_fire |
| 18 | `chaos_bolt` | if=(talent.wither&(soul_shard>=4\|buff.malevolence.up\|pet.infernal.active\|fight_remains<=15))\|(talent.diabolic_ritual&variable.ritual_length>4) |
| 19 | `infernal_bolt` | if=soul_shard<=3 |
| 20 | `incinerate` | — |
...
| 9 | `chaos_bolt` | if=active_enemies<=3 |
