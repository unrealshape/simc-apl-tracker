...
| 7 | `tigers_fury` | if=(cooldown.frantic_frenzy.remains<buff.tigers_fury.duration-1.5\|cooldown.frantic_frenzy.remains>22\|!talent.frantic_frenzy\|spell_targets=1)&!variable.use_custom_timers\|variable.tfNow |
...
| 8 | `variable` | name=holdBerserk,value=variable.firstHoldBerserkCondition\|variable.secondHoldBerserkCondition\|variable.use_custom_timers&variable.nextBSTimer-5>time&!variable.zerkNow |
| 9 | `variable` | name=holdConvoke,value=variable.convokeCountRemaining=1&variable.zerkCountRemaining=1&!buff.bs_inc.up\|variable.zerkCountRemaining>0&talent.ashamanes_guidance&cooldown.bs_inc.remains<50&variable.use_custom_timers |
...
| 14 | `frantic_frenzy` | if=buff.tigers_fury.up&spell_targets>=2\|(spell_targets=1\|fight_style.dungeonslice\|fight_style.dungeonroute)&combo_points<=2+(2*buff.bs_inc.up) |
...
# for practicism, we give a 1s leeway from cast end until start of the fight.
