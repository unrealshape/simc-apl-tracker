...
| 9 | `arcane_torrent` | — |
| 10 | `potion` | if=potion.liquid_luster |
| 11 | `mind_blast` | if=set_bonus.mid2_4pc |
| 12 | `tentacle_slam` | — |
...
| 8 | `power_infusion` | if=(buff.voidform.up\|!talent.voidform)&!buff.power_infusion.up |
| 9 | `halo` | — |
| 10 | `use_item` | name=stormbound_emblem_of_dazar,if=cooldown.voidform.remains<=2&active_dot.shadow_word_pain>=active_dot.vampiric_touch&!buff.crushing_void.up |
...
| 2 | `use_item` | name=hex_lords_dooming_idol,if=buff.power_infusion.up&buff.hex_lords_doom.stack>5\|fight_remains<=30 |
| 3 | `use_item` | name=font_of_venomous_rage |
| 4 | `use_item` | name=vexhuls_everflowing_gland |
| 5 | `use_items` | if=(buff.voidform.up\|buff.power_infusion.remains>=10\|equipped.neural_synapse_enhancer&buff.entropic_rift.up)\|fight_remains<20 |
...
# Crushing Void from Tentacle Slam is reset on pull, open with Mind Blast instead.
...
# Sync Power Infusion with Voidform
...
# Use Holy Nova during Halo to acquire Twist of Fate when Lightburst can heal an ally and Twist of Fate is not active.
...
# Bank Void Volley charges for Resonant Energy, spending them after the proc is reaction-ready, during Voidform, near the next Voidform, at four Crushing Void stacks, or near fight end
