...
| 12 | `variable` | name=nonsteroid_trinket_equipped,op=set,value=equipped.mereldars_toll\|equipped.perfidious_projector\|equipped.chaotic_nethergate\|equipped.wraps_of_cosmic_madness\|equipped.astalors_anguish_agitator\|equipped.litany_of_lightblind_wrath\|equipped.gloom_spattered_dreadscale |
| 13 | `snapshot_stats` | — |
| 14 | `mirror_image` | — |
...
| 3 | `use_item` | name=vaelgors_final_stare,if=buff.combustion.remains>6&buff.bloodlust.up\|fight_remains<20 |
| 4 | `use_item` | name=emberwing_feather,if=buff.combustion.remains>6\|fight_remains<20 |
| 5 | `use_item` | name=vaelgors_final_stare,if=buff.combustion.remains>6\|fight_remains<20 |
| 6 | `use_item` | name=galactic_gladiators_badge_of_ferocity,if=buff.combustion.remains>6\|fight_remains<20 |
| 7 | `use_item` | name=ever_collapsing_void_fissure,if=buff.combustion.remains>6\|fight_remains<20 |
| 8 | `use_item` | name=nevermelting_ice_crystal,if=buff.combustion.remains>6\|fight_remains<20 |
| 9 | `use_item` | name=freightrunners_flask,if=buff.combustion.remains>6\|fight_remains<20 |
...
| 4 | `fire_blast` | use_off_gcd=1,use_while_casting=1,if=cooldown_react&!buff.hot_streak.react&scorch_execute.active&(hot_streak_spells_in_flight+buff.heating_up.react=0)&action.scorch.executing&buff.heat_shimmer.down&gcd.remains<gcd.max |
...
| 7 | `fire_blast` | use_off_gcd=1,use_while_casting=1,if=talent.frostfire_bolt&target.health.pct<30&buff.combustion.down&cooldown.combustion.remains>5 |
...
| 5 | `scorch` | if=buff.combustion.down&(scorch_execute.active\|active_enemies>=4) |
...
| 8 | `scorch` | if=talent.scald&scorch_execute.active\|buff.heat_shimmer.react&(firestarter.active\|prev_gcd.1.pyroblast\|prev_gcd.1.flamestrike) |
| 9 | `fireball` | — |
...
# Vaelgor is prioritised during Bloodlust.
